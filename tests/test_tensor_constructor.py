...
def test_constructor_basic():
    ...
    t = tensor.Tensor(data, np.float32, True, (2, 3))
...
def test_constructor_with_grads():
    ...
    t = tensor.Tensor(data, np.float32, True, (2, 2), _grads=grads)
...
def test_constructor_with_children():
    ...
    children = {tensor.Tensor(np.array([[0.5, 1.0]]), np.float32, True, (1, 2))}

    t = tensor.Tensor(data, np.float32, True, (1, 2), _children=children)
...
def test_constructor_with_op():
    ...
    t = tensor.Tensor(data, np.float32, True, (1, 2), _op='+')
...
def test_constructor_shape_validation():
    ...
    with pytest.raises(ValueError, match='Shape of data and shape and not compatible'):
        tensor.Tensor(data, np.float32, False, (2, 2))  # 4 elements vs 6 elements

    # Valid shape (1D)
    t_1d = tensor.Tensor(data, np.float32, False, (6,))
    assert t_1d.shape == (6,)
...
def test_constructor_requires_grad():
    ...
    t_true = tensor.Tensor(data, np.float32, True, (1, 2))
    assert t_true.requires_grad is True

    # With requires_grad=False
    t_false = tensor.Tensor(data, np.float32, False, (1, 2))
...
def test_constructor_empty_grads():
    ...
    t = tensor.Tensor(data, np.float32, True, (1, 2), _grads=np.array([]))
...
def test_constructor_empty_children():
    ...
    t = tensor.Tensor(data, np.float32, True, (1, 2), _children=set())
...
def test_constructor_1d_tensor():
    ...
    t = tensor.Tensor(data, np.float32, False, (4,))
...
def test_constructor_float64():
    ...
    t = tensor.Tensor(data, np.float64, False, (1, 2))
...
def test_constructor_int32():
    ...
    t = tensor.Tensor(data, np.int32, False, (1, 2))
...
def test_constructor_backward_function():
    ...
    data = np.array([[1, 2]], dtype=np.float32)
    t = tensor.Tensor(data, np.float32, True, (1, 2))
...
def test_constructor_max_dim():
    ...
    data = np.array([[1, 2]], dtype=np.float32)
    t = tensor.Tensor(data, np.float32, False, (1, 2))
