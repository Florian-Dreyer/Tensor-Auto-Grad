...
def test_shape_property():
    ...
    t_2d = tensor.Tensor(data_2d, np.float32, False, (2, 3))
    assert t_2d.shape == (2, 3)

    # Test 1D tensor
    data_1d = np.array([1, 2, 3, 4], dtype=np.float32)
    t_1d = tensor.Tensor(data_1d, np.float32, False, (4,))
    assert t_1d.shape == (4,)

    # Test 3D tensor
    data_3d = np.array([[[1, 2], [3, 4]], [[5, 6], [7, 8]]], dtype=np.float32)
    t_3d = tensor.Tensor(data_3d, np.float32, False, (2, 2, 2))
...
def test_dtype_property():
    ...
    t_float32 = tensor.Tensor(data_float32, np.float32, False, (1, 2))
    assert t_float32.dtype == np.float32

    # Test int32
    data_int32 = np.array([[1, 2]], dtype=np.int32)
    t_int32 = tensor.Tensor(data_int32, np.int32, False, (1, 2))
    assert t_int32.dtype == np.int32

    # Test float64
    data_float64 = np.array([[1.0, 2.0]], dtype=np.float64)
    t_float64 = tensor.Tensor(data_float64, np.float64, False, (1, 2))
...
def test_properties_with_different_dtypes():
    ...
    t_int = tensor.Tensor(data_int, np.int32, False, (1, 2))
    assert t_int.dtype == np.int32
    assert t_int.shape == (1, 2)

    # Test float64
    data_float = np.array([[1.0, 2.0]], dtype=np.float64)
    t_float = tensor.Tensor(data_float, np.float64, False, (1, 2))
